...
Say you’re provisioning a new cluster named `devcluster`. When ClusterAPI creates it, a `Secret` named `devcluster-kubeconfig` is created in the cluster namespace (in our example we create the cluster in a devcluster namespace).
